...
function sendSession(session) {
    if (clients[session.sid]) {
        ...
        buf.writeUInt8(define_1.default.Back_To_Front.applySession, 4);
        msgBuf.copy(buf, 5);
...
function sendMsgByUidSid(cmdIndex, msg, uids, sids) {
    ...
    for (var sid in group) {
        if (clients[sid]) {
            if (!msgBuf) {
                msgBuf = msgCoder_1.encodeData(cmdIndex, msg);
            }
            var buf = msgCoder_1.encodeRemoteData(group[sid], cmdIndex, msgBuf);

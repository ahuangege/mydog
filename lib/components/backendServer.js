...
var backend_socket = /** @class */ (function () {
    function backend_socket(socket) {
        ...
        this.registerTimer = setTimeout(function () {
            app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.backendServer, "register time out, close the socket: " + socket.socket.remoteAddress);
            socket.close();
        }, 10000);
    ...
    backend_socket.prototype.onData = function (data) {
        ...
        if (type === 3 /* msg */) {
            if (!this.registered) {
                app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.backendServer, "get data before registerd, close the socket" + this.socket.socket.remoteAddress);
                this.socket.close();
            }
            try {
            ...
            app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.backendServer, "illegal data, close the socket" + this.socket.socket.remoteAddress + " " + this.sid);
            this.socket.close();
        }
    };
    /**
    ...
    backend_socket.prototype.register_handle = function (_data) {
        ...
        try {
            ...
            app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.backendServer, "register JSON parse err , close the socket: " + this.socket.socket.remoteAddress);
            this.socket.close();
            return;
        }
        if (data.serverToken !== app.serverToken) {
            app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.backendServer, "illegal register token, close the socket: " + this.socket.socket.remoteAddress);
            this.socket.close();
            return;
        }
        ...
        app.logger(interfaceDefine_1.loggerType.info, interfaceDefine_1.componentName.backendServer, "get a new frontend server named " + this.sid);
    };
    /**
     * 心跳
     */
    backend_socket.prototype.heartBeat_handle = function () {
        ...
        this.heartBeatTimer = setTimeout(function () {
            app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.backendServer, "heartbeat timeout, close the frontend server named " + self.sid);
            self.socket.close();

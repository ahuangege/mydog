...
var backend_socket = /** @class */ (function () {
    ...
    backend_socket.prototype.onData = function (data) {
        ...
        if (type === define_1.default.Front_To_Back.msg) {
            this.msg_handle(data);
        }
        else if (type === define_1.default.Front_To_Back.register) {
            this.register_handle(data);
        }
        else if (type === define_1.default.Front_To_Back.heartbeat) {
            this.heartBeat_handle();
        }
        else {
            app.logger(interfaceDefine_1.loggerType.debug, interfaceDefine_1.componentName.backendServer, "illegal data : " + this.sid);
            this.socket.close();
        }
    };
    /**
    ...
    backend_socket.prototype.register_handle = function (_data) {
        ...
        if (data.serverToken !== app.serverToken) {
            app.logger(interfaceDefine_1.loggerType.debug, interfaceDefine_1.componentName.backendServer, "illegal register token");
            this.socket.close();
            return;
        }
    ...
    backend_socket.prototype.heartBeat_handle = function () {
        ...
        this.heartBeatTimer = setTimeout(function () {
            ...
            self.socket.close();
        }, define_1.default.Time.Remote_Heart_Beat_Time * 1000 * 2);
    };
    /**
     * 收到前端服务器消息
     * @param msgBuf
    ...
    backend_socket.prototype.callback = function (cmdId, uid) {
        ...
        return function (msg) {
            ...
            var msgBuf = msgCoder_1.encodeData(cmdId, msg);
            var buf = msgCoder_1.encodeRemoteData([uid], cmdId, msgBuf);

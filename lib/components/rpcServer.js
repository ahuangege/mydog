...
var rpc_server_proxy = /** @class */ (function () {
    ...
    rpc_server_proxy.prototype.register_handle = function (_data) {
        ...
        try {
            ...
            app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.rpcServer, "JSON parse error，close it");
            this.socket.close();
            return;
        }
        if (data.serverToken !== app.serverToken) {
            app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.rpcServer, "illegal token, it");
            this.socket.close();
            return;
        }
        if (!!servers[data.sid]) {

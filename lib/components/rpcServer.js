...
class RpcServerSocket {
    constructor(app, socket) {
        ...
        this.registerTimer = setTimeout(function () {
            app.logger("error" /* error */, `register timeout, close the rpc socket: ${socket.remoteAddress}`);
            socket.close();
        }, 5000);
        let rpcConfig = app.someconfig.rpc || {};
    ...
    onRegisterData(data) {
        try {
            ...
            if (type === 1 /* register */) {
                ...
                this.app.logger("error" /* error */, `illegal rpc register, close the rpc socket: ${this.socket.remoteAddress}`);
                this.socket.close();
            }
        }
        catch (e) {
    ...
    onData(data) {
        try {
            ...
            if (type === 4 /* clientMsgIn */) {
                ...
                this.app.logger("error" /* error */, `illegal data type, close rpc client named: ${this.id}`);
                this.socket.close();
            }
        }
        catch (e) {
    ...
    onClose() {
        ...
        this.app.logger("error" /* error */, `a rpc client disconnected: ${this.id}, ${this.socket.remoteAddress}`);
    }
    /**
     * 注册
     */
    registerHandle(msg) {
        ...
        try {
            ...
            this.app.logger("error" /* error */, `JSON parse error，close the rpc socket: ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        if (data.serverToken !== serverToken) {
            this.app.logger("error" /* error */, `illegal serverToken, close the rpc socket: ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        if (this.app.rpcPool.hasSocket(data.id)) {
            this.app.logger("error" /* error */, `already has a rpc client named: ${data.id}, close it, ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        ...
        this.app.logger("info" /* info */, `get new rpc client named: ${this.id}`);
        // 注册成功，回应
    ...
    heartbeatHandle() {
        ...
        this.heartbeatTimer = setTimeout(function () {
            self.app.logger("warn" /* warn */, `heartBeat time out, close it: ${self.id}`);
            self.socket.close();
        }, heartbeat * 1000 * 2);
    }
    /**

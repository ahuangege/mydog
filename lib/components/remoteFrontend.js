...
function doRemote(msgBuf, session, serverType) {
    ...
    tmpRouter(app, session, serverType, function (sid) {
        ...
        if (!client || !client.isLive) {
            app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.remoteFrontend, app.serverId + " has no backend server " + sid);
            return;
        }
...
var remote_frontend_client = /** @class */ (function () {
    ...
    remote_frontend_client.prototype.doConnect = function (delay) {
        ...
        this.connect_timer = setTimeout(function () {
            self.socket = new tcpClient_1.TcpClient(self.port, self.host, function () {
                app.logger(interfaceDefine_1.loggerType.info, interfaceDefine_1.componentName.remoteFrontend, "connect to backend server " + self.id + " success");
                // 注册
            ...
            app.logger(interfaceDefine_1.loggerType.info, interfaceDefine_1.componentName.remoteFrontend, "try to connect to backend server named " + self.id);
            ...
            self.socket.on("close", function () {
                ...
                app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.remoteFrontend, " socket closed, reconnect the backend server " + self.id + " later");
    ...
    remote_frontend_client.prototype.heartbeat = function () {
        var self = this;
        var timeDelay = define.some_config.Time.Remote_Heart_Beat_Time * 1000 - 5000 + Math.floor(5000 * Math.random());
     ...
     * 心跳回应
     */
    remote_frontend_client.prototype.heartbeat_timeout = function () {
        var self = this;
        this.heartbeat_timeout_timer = setTimeout(function () {
            app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.remoteFrontend, "the heartbeat with backendserver " + self.id + " timeout, close it");
            self.socket.close();
    ...
    remote_frontend_client.prototype.data_switch = function (msg) {
        ...
        else if (type === 3 /* heartbeatResponse */) {

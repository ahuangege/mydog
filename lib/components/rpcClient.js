...
class RpcClientSocket {
    ...
    doConnect(delay) {
        ...
        this.connectTimer = setTimeout(() => {
            let connectCb = function () {
                self.app.logger("info" /* info */, `connect to rpc server success: ${self.id}`);
                // 注册
            ...
            self.app.logger("info" /* info */, `try to connect to rpc server: ${self.id}`);
        }, delay);
    }
    onClose() {
        ...
        this.heartbeatTimeoutTimer = null;
        this.socket = null;
        this.app.logger("error" /* error */, `socket closed, reconnect the rpc server later: ${this.id}`);
    ...
    heartbeatTimeoutStart() {
        ...
        this.heartbeatTimeoutTimer = setTimeout(function () {
            self.app.logger("error" /* error */, `heartbeat timeout, close the rpc socket: ${self.id}`);
            self.socket.close();

...
class ClientManager {
    ...
    addClient(client) {
        if (client.session) {
            this.app.logger("warn" /* warn */, "the I_client has already been added, close it");
            client.close();
            return;
        }
        this.app.clientNum++;
    ...
    handleMsg(client, msgBuf) {
        try {
            if (!client.session) {
                this.app.logger("warn" /* warn */, "cannot handle msg before registered, close it");
                client.close();
                return;
            }
    ...
    doRemote(msg, session, cmdArr) {
        ...
        if (!this.app.rpcPool.hasSocket(id)) {
            this.app.logger("warn" /* warn */, "doRemote: has no socket");
            return;
        }
        let svr = this.app.serversIdMap[id];
        if (svr.frontend || svr.serverType !== cmdArr[0]) {
            this.app.logger("warn" /* warn */, "doRemote: illegal remote");
            return;
        }

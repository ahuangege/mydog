...
function startServer(cb) {
    tcpServer_1.default(app.port, define.some_config.SocketBufferMaxLen, true, startCb, newClientCb);
    function startCb() {
...
var UnregSocket_proxy = /** @class */ (function () {
    ...
    UnregSocket_proxy.prototype.registerTimeout = function () {
        ...
        this.registerTimer = setTimeout(function () {
            app.logger("error" /* error */, appUtil_1.concatStr("register timeout, close it, " + self.socket.remoteAddress));
            self.socket.close();
        }, 10000);
    };
    UnregSocket_proxy.prototype.onData = function (_data) {
        ...
        try {
            ...
            app.logger("error" /* error */, appUtil_1.concatStr("unregistered socket, JSON parse error, close it, ", socket.remoteAddress));
            socket.close();
            return;
        }
        // 第一个数据包必须是注册
        if (!data || data.T !== 1 /* register */) {
            app.logger("error" /* error */, appUtil_1.concatStr("unregistered socket, illegal data, close it, ", socket.remoteAddress));
            socket.close();
            return;
        }
        // 是服务器？
        if (data.serverToken) {
            if (data.serverToken !== serverToken) {
                app.logger("error" /* error */, appUtil_1.concatStr("illegal serverToken, close it, ", socket.remoteAddress));
                socket.close();
                return;
            }
            if (!data.serverType || !data.serverInfo || !data.serverInfo.id || !data.serverInfo.host || !data.serverInfo.port) {
                app.logger("error" /* error */, appUtil_1.concatStr("illegal serverInfo, close it, ", socket.remoteAddress));
                socket.close();
                return;
            }
            this.registerOk();
        ...
        if (data.cliToken) {
            if (data.cliToken !== cliToken) {
                app.logger("error" /* error */, appUtil_1.concatStr("illegal cliToken, close it, ", socket.remoteAddress));
                socket.close();
                return;
            }
            this.registerOk();
        ...
        app.logger("error" /* error */, appUtil_1.concatStr("illegal socket, close it, " + socket.remoteAddress));
        socket.close();
    };
    UnregSocket_proxy.prototype.onClose = function () {
        clearTimeout(this.registerTimer);
        app.logger("error" /* error */, appUtil_1.concatStr("unregistered socket closed, ", this.socket.remoteAddress));
    };
...
var Master_ServerProxy = /** @class */ (function () {
    ...
    Master_ServerProxy.prototype.init = function (data) {
        ...
        if (!!servers[data.serverInfo.id]) {
            app.logger("error" /* error */, appUtil_1.concatStr("already has a monitor named ", data.serverInfo.id, ", close it, ", socket.remoteAddress));
            socket.close();
            return;
        }
        ...
        app.logger("info" /* info */, appUtil_1.concatStr("get a new monitor named ", this.sid, ", ", this.socket.remoteAddress));
    };
    Master_ServerProxy.prototype.heartbeatTimeout = function () {
        var self = this;
        clearTimeout(this.heartbeatTimeoutTimer);
        this.heartbeatTimeoutTimer = setTimeout(function () {
            app.logger("error" /* error */, appUtil_1.concatStr("heartbeat timeout, close the monitor named ", self.sid, ", " + self.socket.remoteAddress));
            self.socket.close();
    ...
    Master_ServerProxy.prototype.onData = function (_data) {
        ...
        try {
            ...
            app.logger("error" /* error */, appUtil_1.concatStr("JSON parse error，close the monitor named ", this.sid, ", ", this.socket.remoteAddress));
            this.socket.close();
            return;
        }
        try {
            ...
            app.logger("error" /* error */, appUtil_1.concatStr("handle msg error, close it, ", this.sid, ", ", this.socket.remoteAddress, "\n", err.stack));
            this.socket.close();
        }
    };
    Master_ServerProxy.prototype.onClose = function () {
        ...
        app.logger("error" /* error */, appUtil_1.concatStr("a monitor disconnected  ", this.sid, ", ", this.socket.remoteAddress));
    };
...
var Master_ClientProxy = /** @class */ (function () {
    ...
    Master_ClientProxy.prototype.init = function () {
        ...
        socket.on('data', this.onData.bind(this));
        socket.on('close', this.onClose.bind(this));
        app.logger("info" /* info */, appUtil_1.concatStr("get a new cli, ", socket.remoteAddress));
    };
    Master_ClientProxy.prototype.heartbeatTimeOut = function () {
        var self = this;
        clearTimeout(this.heartbeatTimer);
        this.heartbeatTimer = setTimeout(function () {
            app.logger("error" /* error */, appUtil_1.concatStr("heartbeat timeout, close the cli, " + self.socket.remoteAddress));
            self.socket.close();
        }, define.some_config.Time.Monitor_Heart_Beat_Time * 1000 * 2);
    };
    Master_ClientProxy.prototype.onData = function (_data) {
        ...
        try {
            ...
            app.logger("error" /* error */, appUtil_1.concatStr("JSON parse error，close the cli, " + this.socket.remoteAddress));
            this.socket.close();
            return;
        }
        try {
            if (data.T === 2 /* heartbeat */) {
                ...
                app.logger("info" /* info */, appUtil_1.concatStr("master get command from the cli, " + this.socket.remoteAddress + " ==> " + JSON.stringify(data)));
                masterCli.deal_cli_msg(this, data);
            }
            else {
                app.logger("error" /* error */, appUtil_1.concatStr("the cli illegal data type close it, " + this.socket.remoteAddress));
                this.socket.close();
            }
        }
        catch (e) {
            app.logger("error" /* error */, appUtil_1.concatStr("cli handle msg err, close it " + this.socket.remoteAddress + "\n" + e.stack));
            this.socket.close();
        }
    };
    ...
    Master_ClientProxy.prototype.onClose = function () {
        clearTimeout(this.heartbeatTimer);
        app.logger("info" /* info */, appUtil_1.concatStr("a cli disconnected, " + this.socket.remoteAddress));
    };

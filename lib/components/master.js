...
var servers = {};
var serversDataTmp = { "T": define_1.default.Master_To_Monitor.addServer, "serverInfoIdMap": {} };
var masterCli;
var app;
...
function startServer(cb) {
    ...
    function newClientCb(socket) {
        ...
        socket.registerTimer = setTimeout(function () {
            app.logger(interfaceDefine_1.loggerType.debug, interfaceDefine_1.componentName.master, "register time out, close it");
            socket.close();
        }, 10000);
...
function heartBeatTimeOut(socket) {
    ...
    socket.heartBeatTimer = setTimeout(function () {
        app.logger(interfaceDefine_1.loggerType.debug, interfaceDefine_1.componentName.master, "heartbeat time out, close it");
        socket.close();
    }, define_1.default.Time.Monitor_Heart_Beat_Time * 1000 * 2);
}
/**
 * socket尚未注册时，关闭的回调
 */
...
function unRegSocketMsgHandle(socket, _data) {
    ...
    try {
        ...
        app.logger(interfaceDefine_1.loggerType.debug, interfaceDefine_1.componentName.master, "JSON parse error, close it");
        socket.close();
        return;
    }
    if (!data || data.T !== define_1.default.Monitor_To_Master.register) {
        app.logger(interfaceDefine_1.loggerType.debug, interfaceDefine_1.componentName.master, "illegal data, close it");
        socket.close();
        return;
    }
    // 判断是服务器，还是cli
    if (data.hasOwnProperty("serverToken")) {
        if (data.serverToken !== app.serverToken || !data.serverType || !data.serverInfo
            ...
            app.logger(interfaceDefine_1.loggerType.debug, interfaceDefine_1.componentName.master, "illegal monitor, close it");
            socket.close();
            return;
        }
    ...
    if (data.hasOwnProperty("clientToken")) {
        if (data.clientToken !== app.clientToken) {
            app.logger(interfaceDefine_1.loggerType.debug, interfaceDefine_1.componentName.master, "illegal cli, close it");
            socket.close();
            return;
        }
    ...
    app.logger(interfaceDefine_1.loggerType.debug, interfaceDefine_1.componentName.master, "illegal socket, close it");
    socket.close();
}
;
/**
...
var Master_ServerProxy = /** @class */ (function () {
    ...
    Master_ServerProxy.prototype.init = function (data) {
        ...
        // 构造新增服务器的消息
        var socketInfo = {
            "T": define_1.default.Master_To_Monitor.addServer,
            "serverInfoIdMap": {}
        };
    ...
    Master_ServerProxy.prototype.processMsg = function (_data) {
        ...
        try {
            ...
            app.logger(interfaceDefine_1.loggerType.debug, interfaceDefine_1.componentName.master, "JSON parse error，close the monitor named " + this.sid);
            this.socket.close();
            return;
        }
        if (data.T === define_1.default.Monitor_To_Master.heartbeat) {
            heartBeatTimeOut(this.socket);
        }
        else if (data.T === define_1.default.Monitor_To_Master.cliMsg) {
    ...
    Master_ServerProxy.prototype.onClose = function () {
        ...
        var serverInfo = {
            "T": define_1.default.Master_To_Monitor.removeServer,
            "id": this.sid,
...
var Master_ClientProxy = /** @class */ (function () {
    ...
    Master_ClientProxy.prototype.processMsg = function (_data) {
        ...
        try {
            ...
            app.logger(interfaceDefine_1.loggerType.debug, interfaceDefine_1.componentName.master, "JSON parse error，close the cli");
            this.socket.close();
            return;
        }
        if (data.T === define_1.default.Cli_To_Master.heartbeat) {
            heartBeatTimeOut(this.socket);
        }
        else if (data.T === define_1.default.Cli_To_Master.cliMsg) {

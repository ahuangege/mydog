...
var UnregSocket_proxy = /** @class */ (function () {
    ...
    UnregSocket_proxy.prototype.registerTimeout = function () {
        ...
        this.registerTimer = setTimeout(function () {
            app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.master, "the socket connected to master register time out, close the socket: " + self.socket.socket.remoteAddress);
            self.socket.close();
        }, 10000);
    };
    ...
    UnregSocket_proxy.prototype.onClose = function () {
        ...
        app.logger(interfaceDefine_1.loggerType.warn, interfaceDefine_1.componentName.master, "unregistered socket closed: " + this.socket.socket.remoteAddress);
    };
    ...
    Master_ClientProxy.prototype.onData = function (_data) {
        ...
        try {
            if (data.T === 2 /* heartbeat */) {
                ...
                app.logger(interfaceDefine_1.loggerType.info, interfaceDefine_1.componentName.master, "the cli illegal data type close it:  " + this.socket.socket.remoteAddress);
                this.socket.close();
            }
        }
        catch (e) {
            app.logger(interfaceDefine_1.loggerType.info, interfaceDefine_1.componentName.master, e);
            this.socket.close();
        }
    };

...
let servers = {};
let serversDataTmp = { "T": 1 /* addServer */, "serverInfoIdMap": {} };
let masterCli;
let app;
...
class UnregSocket_proxy {
    ...
    registerTimeout() {
        ...
        this.registerTimer = setTimeout(function () {
            app.logger("error" /* error */, `register timeout, close it: ${self.socket.remoteAddress}`);
            self.socket.close();
        }, 5000);
    }
    onData(_data) {
        ...
        try {
            ...
            app.logger("error" /* error */, `unregistered socket, JSON parse error, close it: ${socket.remoteAddress}`);
            socket.close();
            return;
        }
        // 第一个数据包必须是注册
        if (!data || data.T !== 1 /* register */) {
            app.logger("error" /* error */, `unregistered socket, illegal data, close it: ${socket.remoteAddress}`);
            socket.close();
            return;
        }
        // 是服务器？
        if (data.serverToken) {
            ...
            if (data.serverToken !== serverToken) {
                app.logger("error" /* error */, `illegal serverToken, close it: ${socket.remoteAddress}`);
                socket.close();
                return;
            }
            if (!data.serverType || !data.serverInfo || !data.serverInfo.id || !data.serverInfo.host || !data.serverInfo.port) {
                app.logger("error" /* error */, `illegal serverInfo, close it: ${socket.remoteAddress}`);
                socket.close();
                return;
            }
            this.registerOk();
        ...
        if (data.cliToken) {
            ...
            if (data.cliToken !== cliToken) {
                app.logger("error" /* error */, `illegal cliToken, close it: ${socket.remoteAddress}`);
                socket.close();
                return;
            }
            this.registerOk();
        ...
        app.logger("error" /* error */, `illegal socket, close it: ${socket.remoteAddress}`);
        socket.close();
    }
    onClose() {
        clearTimeout(this.registerTimer);
        app.logger("error" /* error */, `unregistered socket closed: ${this.socket.remoteAddress}`);
    }
    registerOk() {
        clearTimeout(this.registerTimer);
...
class Master_ServerProxy {
    ...
    init(data) {
        ...
        if (!!servers[data.serverInfo.id]) {
            app.logger("error" /* error */, `already has a monitor named: ${data.serverInfo.id}, close it: ${socket.remoteAddress}`);
            socket.close();
            return;
        }
        ...
        app.logger("info" /* info */, `get a new monitor named: ${this.sid}, ${this.socket.remoteAddress}`);
    }
    heartbeatTimeout() {
        let self = this;
        clearTimeout(this.heartbeatTimeoutTimer);
        this.heartbeatTimeoutTimer = setTimeout(function () {
            app.logger("error" /* error */, `heartbeat timeout, close the monitor named: ${self.sid}, ${self.socket.remoteAddress}`);
            self.socket.close();
    ...
    onData(_data) {
        ...
        try {
            ...
            app.logger("error" /* error */, `JSON parse error，close the monitor named: ${this.sid}, ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        try {
            ...
            app.logger("error" /* error */, `handle msg error, close it: ${this.sid}, ${this.socket.remoteAddress}\n${err.stack}`);
            this.socket.close();
        }
    }
    onClose() {
        ...
        app.logger("error" /* error */, `a monitor disconnected: ${this.sid}, ${this.socket.remoteAddress}`);
    }
}
...
class Master_ClientProxy {
    ...
    init() {
        ...
        socket.on('data', this.onData.bind(this));
        socket.on('close', this.onClose.bind(this));
        app.logger("info" /* info */, `get a new cli: ${socket.remoteAddress}`);
    }
    heartbeatTimeOut() {
        let self = this;
        clearTimeout(this.heartbeatTimer);
        this.heartbeatTimer = setTimeout(function () {
            app.logger("error" /* error */, `heartbeat timeout, close the cli: ${self.socket.remoteAddress}`);
            self.socket.close();
        }, define.some_config.Time.Monitor_Heart_Beat_Time * 1000 * 2);
    }
    onData(_data) {
        ...
        try {
            ...
            app.logger("error" /* error */, `JSON parse error，close the cli: ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        try {
            if (data.T === 2 /* heartbeat */) {
                ...
                app.logger("info" /* info */, `master get command from the cli: ${this.socket.remoteAddress} ==> ${JSON.stringify(data)}`);
                masterCli.deal_cli_msg(this, data);
            }
            else {
                app.logger("error" /* error */, `the cli illegal data type close it: ${this.socket.remoteAddress}`);
                this.socket.close();
            }
        }
        catch (e) {
            app.logger("error" /* error */, `cli handle msg err, close it: ${this.socket.remoteAddress}\n ${e.stack}`);
            this.socket.close();
        }
    }
    send(msg) {
    ...
    onClose() {
        clearTimeout(this.heartbeatTimer);
        app.logger("info" /* info */, `a cli disconnected: ${this.socket.remoteAddress}`);
    }
}
